...
test('workflow.config.js loads and provides required sections', () => {
  ...
  const requiredSections = ['sourceExtensions', 'ignoreDirs', 'maxLines'];
  ...
  if (config.ide) {
    ...
    assert.ok(config.ide.preferIDE !== undefined, 'ide config should have preferIDE setting');
  }
});
...
test('ExperienceStore records and retrieves experiences', () => {
  ...
  // Constructor expects storePath (string, optional, defaults to output/experiences.json)
  ...
  assert.strictEqual(store.experiences[0].id, 'test-exp-001', 'Should preserve ID');
...
test('Observability collects and aggregates metrics', () => {
  ...
  assert.ok(obs.projectId, 'Should have projectId');
  assert.ok(obs.outputDir, 'Should have outputDir');
...
test('RollbackCoordinator manages stage rollback state', () => {
  ...
  assert.ok(typeof coordinator.save === 'function' || typeof coordinator.checkpoint === 'function', 'Should have save/checkpoint method');
