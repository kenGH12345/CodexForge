...
const TECH_PROFILES = [
  ...
  },
  {
    ...
    extensions: ['.java'],
    ignoreDirs: ['node_modules', '.git', 'target', 'build', '.gradle', '.idea', 'out'],
    testCommand: 'mvn test',  // Use 'gradle test' for Gradle projects
    ...
    namingConvention: 'PascalCase for classes, camelCase for methods/variables, UPPER_SNAKE_CASE for constants, reverse-domain for packages',
    stateManagement: () => 'Spring beans (singleton); use immutable DTOs/records for data transfer',
    // P2-2: Framework-specific directory conventions and layering hints
    frameworkHints: {
      'Spring Boot': {
        layers: ['Controller', 'Service', 'Repository', 'Entity', 'DTO', 'Config'],
        dirConvention: 'src/main/java/<group>/<artifact>/{controller,service,repository,entity,dto,config}/',
        configFiles: ['application.yml', 'application.properties'],
        testConvention: 'src/test/java/ mirrors src/main/java/ structure',
      },
      'Quarkus': {
        layers: ['Resource', 'Service', 'Repository', 'Entity'],
        dirConvention: 'src/main/java/<group>/{resource,service,repository,entity}/',
        configFiles: ['application.properties', 'application.yaml'],
      },
      'Maven Multi-Module': {
        dirConvention: '<root>/{module-api,module-core,module-web}/src/main/java/',
        buildHint: 'mvn install from root; modules declared in parent pom.xml',
      },
    },
    projectStructure: [
      'src/main/java/    - Production source code (package hierarchy)',
    ...
    skills: [
      { name: 'workflow-orchestration', description: 'Multi-agent workflow orchestration SOP', domains: ['workflow', 'orchestration'] },
      { name: 'code-review',            description: 'Code review checklist and best practices', domains: ['quality', 'review'] },
      { name: 'java-dev',               description: 'Java development patterns (Spring Boot, Quarkus, JPA)', domains: ['backend', 'java', 'spring'] },
    ],
  ...
  },
  {
    ...
    namingConvention: 'snake_case for functions/variables/files/modules, PascalCase for types/traits/enums, SCREAMING_SNAKE_CASE for constants',
    stateManagement: () => 'Ownership model; Arc<Mutex<T>> or Arc<RwLock<T>> for shared state; channels for message passing',
    // P2-2: Framework-specific directory conventions and layering hints
    frameworkHints: {
      'Actix-Web': {
        layers: ['Handler', 'Service', 'Model', 'Middleware', 'Config'],
        dirConvention: 'src/{handlers,services,models,middleware,config}/',
        configFiles: ['.env', 'config.toml'],
        entryPoint: 'src/main.rs',
      },
      'Axum': {
        layers: ['Handler', 'Service', 'Model', 'Middleware', 'State'],
        dirConvention: 'src/{handlers,services,models,middleware,state}/',
        entryPoint: 'src/main.rs',
      },
      'Cargo Workspace': {
        dirConvention: '<root>/{crate-a,crate-b}/src/ with shared Cargo.toml [workspace]',
        buildHint: 'cargo build from workspace root; crates declared in [workspace].members',
      },
    },
    projectStructure: [
      'src/          - Source code (main.rs or lib.rs as entry)',
      ...
      'Cargo.toml    - Package manifest and dependency management',
    ],
    skills: [
      { name: 'workflow-orchestration', description: 'Multi-agent workflow orchestration SOP', domains: ['workflow', 'orchestration'] },
      { name: 'code-review',            description: 'Code review checklist and best practices', domains: ['quality', 'review'] },
      { name: 'rust-dev',               description: 'Rust development patterns (Actix, Axum, Tokio, Serde)', domains: ['systems', 'rust', 'backend'] },
    ],
  ...
  },
  {
    ...
    ignoreDirs: ['node_modules', '.git', 'build', 'cmake-build-debug', 'cmake-build-release', '.cache', 'out', '.vscode'],
    ...
    namingConvention: 'PascalCase for classes, camelCase or snake_case for functions/variables (project-dependent), UPPER_SNAKE_CASE for macros/constants',
    stateManagement: () => 'RAII + smart pointers (unique_ptr/shared_ptr); avoid raw new/delete',
    // P2-2: Framework-specific directory conventions and layering hints
    frameworkHints: {
      'Qt': {
        layers: ['Widget', 'Model', 'Delegate', 'Dialog', 'MainWindow'],
        dirConvention: 'src/{widgets,models,delegates,dialogs}/',
        configFiles: ['CMakeLists.txt', '*.pro', '*.qrc'],
        buildHint: 'cmake -B build -DCMAKE_PREFIX_PATH=<Qt_install_path> && cmake --build build',
      },
      'CMake Modern': {
        dirConvention: 'src/ include/ tests/ cmake/ with per-target CMakeLists.txt',
        buildHint: 'cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build',
      },
      'vcpkg / Conan': {
        configFiles: ['vcpkg.json', 'conanfile.txt', 'conanfile.py'],
        buildHint: 'vcpkg integrate install; or conan install . --output-folder=build',
      },
    },
    projectStructure: [
      ...
      'cmake/        - CMake modules and Find*.cmake scripts',
    ...
    skills: [
      { name: 'workflow-orchestration', description: 'Multi-agent workflow orchestration SOP', domains: ['workflow', 'orchestration'] },
      { name: 'code-review',            description: 'Code review checklist and best practices', domains: ['quality', 'review'] },
      { name: 'cpp-dev',                description: 'C/C++ development patterns (CMake, Qt, vcpkg, Conan)', domains: ['systems', 'cpp', 'desktop'] },
    ],
