...
try {
        ...
        // Ignore — temp file cleanup failure is not critical
      }
  ...
  _buildPRDescription({ title, body, baseBranch, head, labels, reviewers, draft }) {
    ...
    lines.push('', '---');
lines.push(`*Generated by CODEX FORGE at ${new Date().toISOString()}*`);
...
// Clean up temp body file
      try {
        fs.unlinkSync(path.join(this.repoPath, '.git', 'PR_BODY_TMP'));
      } catch (e) {
        // Ignore — temp file cleanup failure is not critical
      }
...
try {
        fs.unlinkSync(path.join(this.repoPath, '.git', 'MR_BODY_TMP'));
      } catch (e) {
        // Ignore — temp file cleanup failure is not critical
      }
