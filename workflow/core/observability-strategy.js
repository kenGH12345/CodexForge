...
/**
 * P3 fix: Generates a cross-session trends report in Markdown format.
 * Provides human-readable summary of long-term evolution patterns.
 *
 * @param {Array} history - Array of session history objects
 * @param {string} outputDir - Output directory for the report
 * @returns {string|null} Path to the generated report, or null if insufficient data
 */
function generateTrendsReport(history, outputDir) {
  if (!history || history.length < 3) {
    console.log(`[ObservabilityStrategy] ⚠️  Insufficient history for trends report (${history?.length || 0} sessions, need ≥3)`);
    return null;
  }

  const trends = computeTrends(history);
  if (!trends) {
    console.log(`[ObservabilityStrategy] ⚠️  Could not compute trends from history`);
    return null;
  }
  ...
  const report = [
    ...
    `> **Sessions analyzed**: ${history.length}`,
    `> **Time span**: ${history.length > 0 ? _formatTimeSpan(history) : 'N/A'}`,
    '',
    '---',
    '',
    '## 📈 Token Consumption',
    ...
    `| Metric | Value |`,
    `|--------|-------|`,
    `| **Trend** | ${_formatTrend(trends.tokenTrend)} |`,
    `| **Average** | ${trends.avgTokensEst ? Math.round(trends.avgTokensEst).toLocaleString() + ' tokens' : 'N/A'} |`,
    `| **Peak** | ${trends.peakTokensEst ? Math.round(trends.peakTokensEst).toLocaleString() + ' tokens' : 'N/A'} |`,
    '',
    '## ⚠️ Error Rate',
    '',
    `| Metric | Value |`,
    `|--------|-------|`,
    `| **Trend** | ${_formatTrend(trends.errorTrend)} |`,
    `| **Average** | ${trends.avgErrorCount != null ? trends.avgErrorCount.toFixed(2) + ' errors/session' : 'N/A'} |`,
    `| **Peak** | ${trends.peakErrorCount != null ? trends.peakErrorCount + ' errors' : 'N/A'} |`,
    '',
    '## ⏱️ Duration',
    '',
    `| Metric | Value |`,
    `|--------|-------|`,
    `| **Trend** | ${_formatTrend(trends.durationTrend)} |`,
    `| **Average** | ${trends.avgDurationMs ? (trends.avgDurationMs / 1000).toFixed(1) + 's' : 'N/A'} |`,
    `| **Peak** | ${trends.peakDurationMs ? (trends.peakDurationMs / 1000 / 60).toFixed(1) + ' min' : 'N/A'} |`,
    '',
    '## 🧪 Test Results',
    '',
    `| Metric | Value |`,
    `|--------|-------|`,
    `| **Pass Rate Trend** | ${_formatTrend(trends.testPassTrend)} |`,
    `| **Average Pass Rate** | ${trends.avgTestPassRate != null ? Math.round(trends.avgTestPassRate * 100) + '%' : 'N/A'} |`,
    '',
    '---',
    '',
    '## 📋 Recommendations',
    '',
    ..._generateRecommendations(trends),
    '',
    '---',
    '',
    '*Auto-generated by ObservabilityStrategy*',
  ].join('\n');

  try {
    const fs = require('fs');
    const path = require('path');
    const reportPath = path.join(outputDir || 'output', 'trends-report.md');
    ...
    console.log(`[ObservabilityStrategy] 📊 Trends report generated: ${reportPath}`);
    return reportPath;
  } catch (err) {
    console.warn(`[ObservabilityStrategy] ⚠️  Could not write trends report: ${err.message}`);
    return null;
  }
}

/**
 * Formats a trend indicator for display.
 */
function _formatTrend(trend) {
...
/**
 * Formats the time span of the history.
 */
function _formatTimeSpan(history) {
  ...
  const days = Math.ceil((new Date(last) - new Date(first)) / (1000 * 60 * 60 * 24));
 ...
 */
function _generateRecommendations(trends) {
  const recs = [];

  if (trends.errorTrend === 'declining') {
    recs.push('- 🔴 **Error rate is increasing**. Consider reviewing recent changes for quality regressions.');
  }
  if (trends.tokenTrend === 'declining') {
    recs.push('- 🟡 **Token consumption is increasing**. Review prompt efficiency and context budgeting.');
  }
  if (trends.testPassTrend === 'declining') {
    recs.push('- 🔴 **Test pass rate is declining**. Focus on fixing flaky tests and improving test coverage.');
  }
  if (trends.durationTrend === 'declining') {
    recs.push('- 🟡 **Session duration is increasing**. Consider optimizing LLM calls or reducing context size.');
  }
  if (trends.errorTrend === 'improving' && trends.testPassTrend === 'improving') {
    recs.push('- 🟢 **Quality metrics are improving**. Keep up the good work!');
  }
  if (trends.tokenTrend === 'improving') {
    recs.push('- 🟢 **Token efficiency is improving**. Current prompting strategy is effective.');
  }

  if (recs.length === 0) {
    recs.push('- ➖ **No significant issues detected**. Continue monitoring trends over time.');
  }
